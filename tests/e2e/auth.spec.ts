...
test("admin access keys redirects unauthenticated users to login", async ({
  page,
}) => {
  await page.goto("/admin/access-keys");

  await expect(page).toHaveURL(/\/login\?next=%2Fadmin%2Faccess-keys$/);
  await expect(page.getByRole("heading", { name: "Sign in" })).toBeVisible();
});

test("admin entitlements redirects unauthenticated users to login", async ({
  page,
}) => {
  await page.goto("/admin/entitlements");

  await expect(page).toHaveURL(/\/login\?next=%2Fadmin%2Fentitlements$/);
  await expect(page.getByRole("heading", { name: "Sign in" })).toBeVisible();
});
...
test("admin change requests redirects unauthenticated users to login", async ({
  page,
}) => {
  await page.goto("/admin/change-requests");

  await expect(page).toHaveURL(/\/login\?next=%2Fadmin%2Fchange-requests$/);
  await expect(page.getByRole("heading", { name: "Sign in" })).toBeVisible();
});

test("admin modules redirects unauthenticated users to login", async ({
  page,
}) => {
  await page.goto("/admin/modules");

  await expect(page).toHaveURL(/\/login\?next=%2Fadmin%2Fmodules$/);
  await expect(page.getByRole("heading", { name: "Sign in" })).toBeVisible();
});

test("admin RAG approval redirects unauthenticated users to login", async ({
  page,
}) => {
  await page.goto("/admin/rag");

  await expect(page).toHaveURL(/\/login\?next=%2Fadmin%2Frag$/);
  await expect(page.getByRole("heading", { name: "Sign in" })).toBeVisible();
});

test("admin audit logs redirects unauthenticated users to login", async ({
  page,
}) => {
